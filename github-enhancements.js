        ...
        enhancementFunctions['pull-request-collapse'] = function () {
            ...
            function enhanceFiles(listener, files) {
                files.forEach(file => {
                    file.addEventListener('dblclick', evt => {
                        ...
                        var btn = header.querySelector('.js-details-target[aria-label="Toggle diff text"]');
                        btn.click();
                        clearSelection();
                    });
                });
            ...
            function enhanceHeader(listener, header) {
                ...
                function toggleFileDetails(toggle) {
                    var files = document.querySelectorAll(`.js-details-target[aria-label="Toggle diff text"][aria-expanded="${toggle}"]`);
                    files.forEach(file => file.click());
                }
                var btn = createButton('Collapse all', 'btn btn-sm btn-outline BtnGroup-item');
                btn.addEventListener('click', () => toggleFileDetails(true));
                btnGroup.appendChild(btn);
                btn = createButton('Expand all', 'btn btn-sm btn-outline BtnGroup-item');
